...
- наслов (текстуална низа од максимум 50 знаци), категорија (текстуална низа од максимум 30 знаци), 
опис (текстуална низа од максимум 100 знаци), цена изразена во евра (реален број).
За оваа класа да се обезбедат:
- Оператор > за споредба на двa огласи според цената
- Оператор << за печатење на податоците за огласот во форма:
[наслов]
[опис]
[цена] evra
Да се развие класа Oglasnik во која се чуваат податоци за:
- име на огласникот (текстуална низа од максимум 20 знаци), низа од огласи (динамички резервирана низа од објекти од класата Oglas),
број на огласи во низата (цел број).
За оваа класа да се обезбедат:
- Оператор += за додавање нов оглас во низата од огласи. Ако цената на огласот што се внесува е негативна, треба да се генерира исклучок 
NegativnaVrednost (објект од класата NegativnaVrednost што посебно треба да се дефинира). Во ваков случај се печати порака 
“Oglasot ima nevalidna vrednost za cenata i nema da bide evidentiran!“ и не се додава во низата.
- Оператор << за печатење на огласите во огласникот. Печатењето треба да биде во следниот формат:
[Име на огласникот]
[наслов1] [опис1] [цена1]
[наслов2] [опис2] [цена2]
 ...
...
Да се обезбедат сите потребни методи за правилно функционирање на програмата.*/
...
public:
    ...
    Oglas(const Oglas &dr){
        strcpy(this->naslov,dr.naslov);
        strcpy(this->kategorija,dr.kategorija);
        strcpy(this->opis,dr.opis);
        this->cena=dr.cena;
    }
    bool operator>(const Oglas &dr)const{
        return dr.cena < cena;
    }
...
public:
    ...
    Oglasnik(char *ime="", Oglas *oglasi= nullptr, int n=0){
        strcpy(this->ime,ime);
        this->n=n;
    ...
    Oglasnik(const Oglasnik &dr){
        ...
        this->n=dr.n;
        this->oglasi=new Oglas[dr.n];
        for(int i=0;i<n;i++){
            this->oglasi[i]=dr.oglasi[i];
        }
    }
    ...
    void najniskaCena(){
        Oglas min=oglasi[0];
        double minCena=min.getCena();
        for(int i=0;i<n;i++){
            if(minCena>oglasi[i].getCena()){
...
int main(){
    ...
    if (tip==1){
        ...
        for (int i = 0; i < n; i++){
            ...
            Oglas o(naslov, kategorija, opis, cena);
            try{ogl+=o;}
            catch(NegativnaVrednost e){e.message();}
        }
        cout<<ogl;
